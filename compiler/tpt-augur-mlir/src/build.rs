...
//! [`tpt_augur_ir::Model`]. Callers must lower via `tpt_augur_ir::lower` and confirm
...
//! are all rejected earlier by `tpt-augur-ir`'s checker).

use std::collections::HashMap;

use tpt_augur_frontend::{BinOp, CmpOp, Expr};
use tpt_augur_ir::{Model, ModelItem};
...
mod tests {
    ...
    fn build(src: &str) -> Graph {
        ...
        let lowered = tpt_augur_ir::lower(&parsed.program);
        assert!(
            !lowered
                .diagnostics
                .iter()
                .any(tpt_augur_ir::Diagnostic::is_error),
            "{:?}",
            lowered.diagnostics
        );
