...
use tpt_augur_ir::lower::known_dist_arity;
use tpt_augur_ir::{
...
fn eval_arithmetic_and_precedence() {
    use tpt_augur_frontend::ast::Stmt;
...
fn eval_undeclared_returns_nan() {
    let r = parse("let x = y");
    let value = match &r.program.statements[0] {
        tpt_augur_frontend::ast::Stmt::Let { value, .. } => eval(value, &Env::new()),
        _ => panic!("expected let"),
    };
    assert!(value.is_nan());
}
...
fn instantiate_dist_known_families() {
    use tpt_augur_frontend::ast::{Expr, Stmt};
...
fn instantiate_unknown_dist_is_none() {
    ...
    let unknown = tpt_augur_frontend::ast::Expr::Call {
        name: "Nope".to_string(),
        args: vec![],
    };
...
fn log_joint_normal_normal_matches_hand_computation() {
    ...
    let prior = tpt_augur_std::Dist::Normal {
        mu: 0.0,
        sigma: 1.0,
    }
    .logp(0.25);
    let like = tpt_augur_std::Dist::Normal {
        mu: 0.25,
        sigma: 1.0,
    }
    .logp(0.5);
...
fn log_joint_beta_binomial_matches_hand_computation() {
    ...
    let prior = tpt_augur_std::Dist::Beta { a: 1.0, b: 1.0 }.logp(0.7);
    let like = tpt_augur_std::Dist::Binomial { n: 10.0, p: 0.7 }.logp(7.0);
    assert!((lp - (prior + like)).abs() < 1e-12, "lp={lp}");
}
...
fn if_gates_observe() {
    ...
    let like_pos = tpt_augur_std::Dist::Normal {
        mu: 0.6,
        sigma: 1.0,
    }
    .logp(1.0);
    let like_neg = tpt_augur_std::Dist::Normal {
        mu: -0.6,
        sigma: 1.0,
    }
    .logp(-1.0);
    let prior_pos = tpt_augur_std::Dist::Normal {
        mu: 0.0,
        sigma: 1.0,
    }
    .logp(0.6);
    let prior_neg = tpt_augur_std::Dist::Normal {
        mu: 0.0,
        sigma: 1.0,
    }
    .logp(-0.6);
