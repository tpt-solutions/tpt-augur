...
//! (see `tpt-augur-mlir`); this crate is the hand-off boundary into the existing
...
pub fn handoff_model(
    ...
    let (tptir, mlir_changes) =
        tpt_augur_mlir::compile_model_to_tptir(model, entry_name, hardware.as_str());
...
pub fn benchmark_throughput(
    ...
    let opts = tpt_augur_runtime::InferOptions {
    ...
    let _result = tpt_augur_runtime::run(model, &opts);
...
mod tests {
    ...
    fn model(src: &str) -> Model {
        ...
        let l = tpt_augur_ir::lower(&p.program);
        assert!(
