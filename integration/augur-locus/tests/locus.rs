...
fn evaluate_strategy_returns_probability_in_unit_interval() {
    let s = Strategy { id: "s1".into(), label: "aggressive".into(), features: vec![1.5, 0.5] };
...
fn probability_increases_with_feature_mass() {
    let weak = Strategy { id: "w".into(), label: "w".into(), features: vec![-2.0] };
    let strong = Strategy { id: "s".into(), label: "s".into(), features: vec![2.0] };
...
fn bridge_ranks_strategies_by_probability() {
    ...
    let weak = Strategy { id: "weak".into(), label: "weak".into(), features: vec![-2.0] };
    let strong = Strategy { id: "strong".into(), label: "strong".into(), features: vec![2.0] };
...
fn evaluate_source_injects_features() {
    ...
    let s = Strategy { id: "x".into(), label: "x".into(), features: vec![0.5] };
...
fn probability_of_success_is_copied_clamp() {
    let p = ProbabilityOfSuccess { value: 0.7, ci_low: 0.5, ci_high: 0.9 };
    let q = p;
    assert_eq!(p, q);
}
