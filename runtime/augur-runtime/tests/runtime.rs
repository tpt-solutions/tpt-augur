...
use augur_runtime::{
    ...
    hmc, mh, pf, vi,
    select_engine, summarize, InferenceResult,
};
...
fn normal_normal_conjugate_hmc() {
    let res = infer(
        ...
        InferOptions { engine: Some(Engine::Hmc), ..Default::default() },
    );
...
fn normal_normal_conjugate_mh() {
    let res = infer(
        "let mu ~ Normal(0, 1)\nobserve Normal(mu, 1) = 0.5",
        InferOptions { engine: Some(Engine::MetropolisHastings), ..Default::default() },
    );
    let mean = res.mean_of("mu").unwrap();
...
fn normal_normal_conjugate_vi() {
    let res = infer(
        "let mu ~ Normal(0, 1)\nobserve Normal(mu, 1) = 0.5",
        InferOptions { engine: Some(Engine::Variational), ..Default::default() },
    );
    let mean = res.mean_of("mu").unwrap();
    assert!((mean - 0.25).abs() < 0.05, "mean={mean}");
}
...
fn beta_binomial_conjugate_pf() {
    let res = infer(
        ...
        InferOptions { engine: Some(Engine::ParticleFilter), ..Default::default() },
    );
...
fn beta_binomial_vi_sanity() {
    ...
    assert!(mean > 0.5, "VI should move toward the likelihood (mean={mean})");
}
...
fn auto_engine_selects_vi_for_high_dimensional() {
    let priors: String = (0..16).map(|i| format!("let x{i} ~ Normal(0, 1)")).collect::<Vec<_>>().join("\n");
    let m = model(&priors);
...
fn engine_from_str_and_as_str() {
    ...
    assert_eq!(Engine::from_str("metropolis-hastings").unwrap(), Engine::MetropolisHastings);
...
fn all_engines_produce_requested_chain_counts() {
    ...
    for engine in [Engine::Hmc, Engine::MetropolisHastings, Engine::Variational, Engine::ParticleFilter] {
        let trace = match engine {
            Engine::Hmc => hmc::run_all(&m, &opts),
            Engine::MetropolisHastings => mh::run_all(&m, &opts),
            Engine::Variational => vi::run_all(&m, &opts),
        ...
        match engine {
            Engine::ParticleFilter => {
                ...
                assert_eq!(trace.chains.len(), opts.num_chains, "{engine:?} chain count");
            }
            Engine::Variational => {
...
fn transforms_roundtrip() {
    use augur_runtime::transforms::{Transform, transform_for};
    ...
    let halfnormal = Expr::Call { name: "HalfNormal".into(), args: vec![] };
    assert!(matches!(transform_for(&halfnormal), Transform::Log));
    let beta = Expr::Call { name: "Beta".into(), args: vec![] };
    assert!(matches!(transform_for(&beta), Transform::Logit { .. }));
    let normal = Expr::Call { name: "Normal".into(), args: vec![] };
...
fn initial_point_uses_typical_points() {
    ...
    assert_eq!(pt.len(), 2);
    assert!(pt[0] > 0.0 && pt[0] < 1.0, "beta init in support: {}", pt[0]);
}
...
fn trace_summary_quantile_ordering_and_rhat() {
    ...
    assert!(s.rhat < 1.2, "rhat should be near 1 for a healthy chain: {}", s.rhat);
    assert!(s.ess > 0.0);
