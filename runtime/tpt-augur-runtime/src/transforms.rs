...
pub fn transform_for(dist_expr: &tpt_augur_frontend::Expr) -> Transform {
    if let tpt_augur_frontend::Expr::Call { name, args, .. } = dist_expr {
        match name.as_str() {
...
fn const_val(e: &tpt_augur_frontend::Expr) -> Option<f64> {
    match e {
        tpt_augur_frontend::Expr::Num(x) => Some(*x),
        tpt_augur_frontend::Expr::Neg(inner) => const_val(inner).map(|x| -x),
        tpt_augur_frontend::Expr::Paren(inner) => const_val(inner),
        _ => None,
    }
}
