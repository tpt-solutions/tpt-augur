...
use tpt_augur_ir::{eval, instantiate_dist, Env, Model, ModelItem};
...
fn apply_support(d: &tpt_augur_std::Dist, v: f64) -> f64 {
    ...
    match d {
        tpt_augur_std::Dist::Beta { .. } => v.clamp(1e-6, 1.0 - 1e-6),
        tpt_augur_std::Dist::HalfNormal { .. }
        | tpt_augur_std::Dist::Gamma { .. }
        | tpt_augur_std::Dist::Exponential { .. } => v.max(1e-6),
        tpt_augur_std::Dist::Uniform { lo, hi } => v.clamp(*lo, *hi),
        _ => v,
    }
}
...
fn collect(
    model: &Model,
    priors: &mut Vec<(tpt_augur_ir::Expr, String)>,
    observes: &mut Vec<(tpt_augur_ir::Expr, tpt_augur_ir::Expr)>,
) {
