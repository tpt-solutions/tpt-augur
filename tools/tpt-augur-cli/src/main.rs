...
fn cmd_run(
    ...
    let result = tpt_augur_runtime::run(&lowered.model, &opts);
...
fn cmd_build(
    ...
    let (tptir, changes) = tpt_augur_mlir::compile_model_to_tptir(&lowered.model, entry, hardware);
...
fn cmd_publish(manifest: &Path, sources: &[PathBuf]) -> Result<ExitCode> {
    let man = tpt_augur_pkg::Manifest::load(manifest)
    ...
    for src in sources {
        ...
        let path = src
            ...
            .to_string();
        files.push(tpt_augur_pkg::PackageFile { path, contents });
    }
    let pkg = tpt_augur_pkg::PublishedPackage {
        manifest: man,
        sources: files,
    };
    let registry = tpt_augur_pkg::Registry::open_default()?;
    registry
        .publish(&pkg)
...
fn print_summary(r: &tpt_augur_runtime::InferenceResult) {
    println!(
        "{:<14}{:>12}{:>12}{:>12}{:>12}{:>10}",
...
fn report_frontend_diags(diags: &[tpt_augur_frontend::Diagnostic]) {
    for d in diags {
        eprintln!(
            "error: {} (span {}-{})",
...
fn report_ir_diags(diags: &[tpt_augur_ir::Diagnostic]) {
    for d in diags {
