...
- [x] Rust workspace layout: `compiler/` (frontend + MLIR passes), `runtime/` (sampling
      execution engine), `stdlib/` (distribution library), `pkg/` (`tpt-augur-pkg`), `integration/`
...
- [x] Evaluate reusing `tptb-core`'s existing parsing/formatting infrastructure instead of a
      ...
      the dependency. <!-- verify: compiler/tpt-augur-frontend/src/parser.rs -->
- [x] `augur fmt` — code formatter <!-- verify: compiler/tpt-augur-frontend/src/format.rs::format_program -->

## Phase 2 — Type System & Uncertainty Propagation (spec.txt §2)

- [x] Distribution types as first-class types in the type system
      <!-- verify: compiler/tpt-augur-ir/src/lower.rs::ModelItem -->
- [x] Automatic uncertainty propagation through standard math operations (`+`, `-`, `*`, `/`,
      composed expressions) <!-- verify: compiler/tpt-augur-ir/src/lower.rs::log_joint -->
- [x] Deterministic/probabilistic interop: standard `if/else` mixed safely with probabilistic
      logic <!-- verify: compiler/tpt-augur-ir/src/lower.rs::log_joint_items -->
- [x] Type-checker with diagnostics (feeds LSP in Phase 6)
      <!-- verify: compiler/tpt-augur-ir/src/lower.rs::lower_stmt -->
- [x] Static analysis for malformed/degenerate distribution parameters
      <!-- verify: compiler/tpt-augur-ir/src/lower.rs::check_degenerate_literal -->
...
- [x] Model-topology analyzer that inspects a program's probabilistic graph to pick an
      inference strategy automatically <!-- verify: runtime/tpt-augur-runtime/src/engine.rs::select_engine -->
- [x] **Hamiltonian Monte Carlo (HMC)** engine <!-- verify: runtime/tpt-augur-runtime/src/hmc.rs::run_all -->
- [x] **Variational Inference (VI)** engine <!-- verify: runtime/tpt-augur-runtime/src/vi.rs::run_all -->
- [x] **Particle Filter** engine <!-- verify: runtime/tpt-augur-runtime/src/pf.rs::run_all -->
- [x] Engine selection heuristics/overrides (manual pragma to force a specific engine)
      <!-- verify: runtime/tpt-augur-runtime/src/engine.rs::InferOptions -->
- [x] Correctness test suite against known-closed-form posteriors
      <!-- verify: runtime/tpt-augur-runtime/src/lib.rs::normal_normal_conjugate_hmc -->
...
- [x] Custom probabilistic MLIR dialect (distributions, sampling ops, inference-graph nodes)
      <!-- verify: compiler/tpt-augur-mlir/src/dialect.rs::Graph -->
- [x] Probabilistic-specific MLIR optimization passes (pre-lowering)
      <!-- verify: compiler/tpt-augur-mlir/src/passes.rs::default_pipeline -->
- [x] Lowering pass: Augur MLIR → TPTIR (`../tpt-gpu/layer3_tptc`)
      <!-- verify: compiler/tpt-augur-mlir/src/codegen.rs::emit_tptir -->
- [x] Integration with `layer7_tptb` toolchain (CLI build/run flow, not a parallel backend)
      <!-- verify: tools/tpt-augur-cli/src/main.rs::cmd_build -->
- [x] Hardware-agnostic dispatch validated across NVIDIA, AMD, and Apple Silicon via existing
      tpt-gpu execution layers — `tpt-augur-tpt` selects a `HardwareTarget`, tags emitted TPTIR
...
- [x] Fearless-concurrency model for parallel MCMC chains — independent chains are
      ...
      <!-- verify: runtime/tpt-augur-runtime/src/hmc.rs::run_all, runtime/tpt-augur-runtime/src/mh.rs::run_all -->
- [x] Interop with tpt-gpu's runtime (`../tpt-gpu/layer4_tptr`) — `tpt-augur-tpt` owns the
      ...
      dialect being registered in `layer3_tptc` <!-- verify: integration/tpt-augur-tpt/src/lib.rs::Handoff -->
- [x] Deterministic-fallback execution path (non-probabilistic code runs without sampling
      overhead) <!-- verify: runtime/tpt-augur-runtime/src/common.rs::eval_deterministic -->
- [x] Runtime error handling/diagnostics surfaced back to source locations
      <!-- verify: tools/tpt-augur-cli/src/main.rs::report_ir_diags -->
...
- [x] Extend/reuse `tptb-lsp` for Augur: distribution type-checking diagnostics
      ...
      built for a different language <!-- verify: compiler/tpt-augur-lsp/src/lib.rs::analyze_document -->
- [x] Inference-graph visualization support (LSP custom request or companion tool)
      ...
      `augur graph` CLI command emits the same DOT <!-- verify: compiler/tpt-augur-lsp/src/lib.rs::inference_graph_dot, tools/tpt-augur-cli/src/main.rs::cmd_graph -->
...
- [x] `tpt-augur-cli` — build/run/test/repl commands (via or alongside `tptb-cli`)
      — `tpt-augur-cli` exists with `run`/`check`/`fmt`/`repl`; full `tptb-cli` integration pending
      <!-- verify: tools/tpt-augur-cli/src/main.rs -->

## Phase 7 — Package Management (spec.txt §3 Package Management)

- [x] `tpt-augur-pkg` wrapper over Cargo/Rust FFI — dependency half implemented
      (renders Augur deps to a Cargo `[dependencies]` table); build/link FFI pending
      <!-- verify: pkg/tpt-augur-pkg/src/lib.rs::to_cargo_deps -->
- [x] Package manifest format for Augur-specific modules
      <!-- verify: pkg/tpt-augur-pkg/src/lib.rs::Manifest -->
- [x] Publish/install flow for Augur module registry
      <!-- verify: pkg/tpt-augur-pkg/src/lib.rs::Registry, tools/tpt-augur-cli/src/main.rs::cmd_publish -->
...
> `tpt-augur-keystone` is not yet listed in the root `Cargo.toml` workspace `members`, so it
> builds standalone (`cargo build -p tpt-augur-keystone` from `integration/tpt-augur-keystone`) but
...
- [x] Native query bindings from Augur models into Keystone DB — typed `tpt_sdk::QueryBuilder`
      ...
      <!-- verify: integration/tpt-augur-keystone/src/lib.rs::KeystoneQuerier -->
- [x] Prior-distribution updates from real-time relational data (Keystone core engine)
      <!-- verify: integration/tpt-augur-keystone/src/lib.rs::prior_from_relational -->
- [x] Prior-distribution updates from vector data (Keystone Prism/vector engine)
      <!-- verify: integration/tpt-augur-keystone/src/lib.rs::prior_from_vector_similarity -->
- [x] Example model demonstrating live prior updates from a Keystone query
      <!-- verify: integration/tpt-augur-keystone/src/lib.rs::model_from_prior -->
- [x] Integration tests against a running Keystone instance — `#[ignore]`d end-to-end test,
      runs against a live Keystone node with `cargo test -p tpt-augur-keystone --test
      live_integration -- --ignored` <!-- verify: integration/tpt-augur-keystone/tests/live_integration.rs -->

## Phase 9 — TPT Locus Integration (spec.txt §4)

- [x] Define the "probability of success" scoring API surface Locus will call into
      <!-- verify: integration/tpt-augur-locus/src/lib.rs::ProbabilityOfSuccess -->
- [x] Expose a stable Augur model-evaluation entry point (strategy → probability output)
      <!-- verify: integration/tpt-augur-locus/src/lib.rs::evaluate_strategy -->
- [x] Coordinate schema/interface with Locus's spec.txt §4 expectations once Locus has code —
      ...
      <!-- verify: integration/tpt-augur-locus/src/lib.rs::LocusAugurBridge -->
- [x] End-to-end integration test once both projects have working builds — exercises a real
      `locus_core::agent::AgentRegistry` <!-- verify: integration/tpt-augur-locus/src/lib.rs::integration_with_locus_agent_registry -->
